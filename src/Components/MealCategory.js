...
class MealCategory extends Component {
    ...
    componentDidMount(){
        axios.get(`/api/meals/${this.props.mealStore.category}`)
            .then(res => {
                this.setState({
                    meals: res.data
                })
        ...
        return (
            <div>
                {this.state.meals.map((meal, index) => {
                    return <Meal deleteMeal={this.deleteMeal} meal={meal} key={`${meal}: ${index}`} />
                })}
            </div>
        )
    }
...
export default connect(mapStatetoProps, {saveMeal})(MealCategory)